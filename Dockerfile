...
# -------------------------------------------
# --- Put Z-Push code from reference ---
# -------------------------------------------
# Put the Z-Push code and move to correct locations
COPY . /tmp/z-push/
RUN cp /tmp/z-push/config/nginx/z-push-php.conf /etc/nginx/snippets/ && \
...
COPY dockerconfig/preflight.sh /usr/local/bin/
COPY dockerconfig/z-push.conf /etc/nginx/http.d/
COPY dockerconfig/fpm-zpush.conf /etc/php${PHP_VERSION}/php-fpm.d/www.conf
COPY dockerconfig/opcache.ini /etc/php${PHP_VERSION}/conf.d/00_opcache.ini
COPY dockerconfig/homepage.html /usr/share/z-push/www/index.html
